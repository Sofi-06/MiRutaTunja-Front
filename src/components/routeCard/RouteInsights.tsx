...
export default function RouteInsights() {
  return (
    <View style={styles.insightsGrid}>
      <View style={[styles.insightPanel, styles.insightPanelBlue]}>
        <Text style={styles.insightTitle}>Rutas recomendadas</Text>
        ...
        <View style={styles.recommendationList}>
          {recommendations.map(([title, description, tag, duration]) => (
            <Pressable key={title} style={styles.recommendationRow}>
              ...
              <Text style={styles.recommendationDuration}>{duration}</Text>
      ...
      <View style={[styles.insightPanel, styles.insightPanelGreen]}>
        <View style={styles.insightTitleRow}>
          <Icon name="history" color={colors.muted} size={18} />
          <Text style={styles.insightTitle}>Últimas consultas</Text>
        </View>
        <View style={styles.recentList}>
          {recentSearches.map(([origin, destination, date]) => (
            <Pressable key={`${origin}-${destination}`} style={styles.recentRow}>
